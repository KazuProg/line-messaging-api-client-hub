...
任意で、**チャネルアクセストークンをサーバー側だけに置いたまま** LINE Messaging API（`api.line.me`）へリクエストできる **`/line-api/` プロキシ**も提供します（後述）。
...
| `LINE_CHANNEL_ACCESS_TOKEN` | いいえ | LINE の「チャネルアクセストークン（長期）」。`/line-api/` プロキシが上流へ付与する `Authorization: Bearer ...` に使用します。未設定の場合、`/line-api/` は 503 を返します。 |
| `LINE_API_BASE_URL` | いいえ | プロキシの転送先オリジン。省略時は `https://api.line.me`。通常は変更不要です。 |
...
- **`/line-api/`** … LINE Messaging API へのプロキシ。チャネルアクセストークン相当の権限で API を呼び出せるため、**インターネットに直接公開しない**（Docker Compose の内部ネットワークなど、信頼できる経路からのみ呼び出す）こと。
...
| * | `/line-api/*` | LINE Messaging API へのリバースプロキシ（`LINE_CHANNEL_ACCESS_TOKEN` が必要） |
...
## LINE API プロキシ `/line-api/`

`https://api.line.me` へのリクエストを本ハブが中継し、**`Authorization: Bearer <LINE_CHANNEL_ACCESS_TOKEN>`** を自動付与します。クライアントは Bearer トークンを持たず、パスだけ本ハブ向けに書き換えます。

- **パスの対応**: 本ハブでは **`/line-api` を除いたパス**がそのまま `LINE_API_BASE_URL`（既定は `https://api.line.me`）に付きます。  
  例: `POST https://あなたのドメイン/line-api/v2/bot/message/push` → 上流は `POST https://api.line.me/v2/bot/message/push`。
- **前提**: 環境変数 `LINE_CHANNEL_ACCESS_TOKEN` が設定されていること。未設定時は **`503 Service Unavailable`**（`LINE API proxy not configured`）。
- **クライアントからの `Authorization`**: プロキシは削除し、サーバー設定のチャネルアクセストークンで上書きします。
- **Body サイズ**: リクエスト Body は **10MB** まで。

**リクエスト例**（プッシュメッセージ）:

```bash
curl -s -X POST "http://localhost:8080/line-api/v2/bot/message/push" \
  -H "Content-Type: application/json" \
  -d '{"to":"USER_ID","messages":[{"type":"text","text":"hello"}]}'
```
